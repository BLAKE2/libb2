...
int blake2b_selftest(void) {
  ...
  for (i = 0; i < KAT_LENGTH; ++i) {
    ...
    int err = 0;
    if ((err = blake2b(hash, BLAKE2B_OUTBYTES, buf, i, key, BLAKE2B_KEYBYTES)) < 0) {
      return -1;
    }
  ...
  /* Test error checking */
  {
    blake2b_state S;
    uint8_t hash[BLAKE2B_OUTBYTES];
    if( !(blake2b_init(&S, BLAKE2B_OUTBYTES + 1) < 0) ) { /* Output too large */
      return -8;
    }
    if( !(blake2b_init_key(&S, BLAKE2B_OUTBYTES, NULL, 1) < 0) ) { /* NULL key */
      return -9;
    }
    if( !(blake2b_init(&S, BLAKE2B_OUTBYTES) == 0) ) { /* OK */
      return -10;
    }
    if( !(blake2b_final(&S, hash, sizeof hash) == 0) ) { /* OK */
      return -11;
    }
    if( !(blake2b_final(&S, hash, sizeof hash) < 0) ) { /* invalid state */
      return -12;
    }
    if( !(blake2b_update(&S, hash, sizeof hash) < 0) ) { /* invalid state */
      return -13;
    }
  }
  return 0;
...
int main(void) {
  int errorcode = blake2b_selftest();
  printf("%s\n", errorcode < 0 ? "error" : "ok");
  return errorcode;
}
#endif
